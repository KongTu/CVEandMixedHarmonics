...
double PbPb_ntrkBinCenter[] = {1, 1, 1, 1, 35, 45, 55, 65, 75, 85};
double PbPb_binCenter[] = {1, 1, 1, 1, 35, 45, 55, 65, 75, 85};
...
*/
	...
	for(int i = 0; i < mult_; i++){
		...
		file1[i] = new TFile(Form("../rootfiles/CVEandMixedHarmonics_PbPb_30_100_v4_%d.root", i+1));
		...
		cout << "ntrk: " << Ntrk[i]->GetMean() << endl;

		if( isNtrk ){

			PbPb_ntrkBinCenter[i] = Ntrk[i]->GetMean();
			PbPb_binCenter[i] = PbPb_ntrkBinCenter[i];
	
		}
		else{

			//PbPb_binCenter[i] = PbPb_centralityBinCenter[i];
			PbPb_binCenter[i] = Ntrk[i]->GetMean();
		}
	...
	for(int i = 0; i < mult_; i++){
		...
		cout << "f_sig_K0s: " << fitParameters_K0s[6] << endl;
	}
	...
	for(int mult = 0; mult < mult_; mult++){
		...
		double c2_ab = meanQaQc*meanQcQb/meanQaQb;
...
*/
	...
	for(int mult = 0; mult < mult_; mult++){
		...
		for(int i = 0; i < 3; i++){
			...
			gr1_Pb[i]->SetPoint(mult, PbPb_binCenter[mult], Pb_value/v2[mult][0]);
			...
			gr1_p[i]->SetPoint(mult, PbPb_binCenter[mult], p_value/v2[mult][1]);
		...
		for(int i = 0; i < 2; i++){
			...
			gr3_Pb[i]->SetPoint(mult, PbPb_binCenter[mult], Pb_value/v2[mult][0]);
			...
			gr3_p[i]->SetPoint(mult, PbPb_binCenter[mult], p_value/v2[mult][1]);
			...
			gr4_Pb[i]->SetPoint(mult, PbPb_binCenter[mult], Pb_value/v2[mult][0]);
			...
			gr4_p[i]->SetPoint(mult, PbPb_binCenter[mult], p_value/v2[mult][1]);
		...
		gr2_Pb->SetPoint(mult, PbPb_binCenter[mult], Pb_value/v2[mult][0]);
		...
		gr2_p->SetPoint(mult, PbPb_binCenter[mult], p_value/v2[mult][1]);
		...
		gr5_Pb->SetPoint(mult, PbPb_binCenter[mult], Pb_value/v2[mult][0]);
		...
		gr5_p->SetPoint(mult, PbPb_binCenter[mult], p_value/v2[mult][1]);
	...
	if( isNtrk ){
		...
		TFile f1("../dataPoints/V0s_PbPb_Ntrk.root","RECREATE");
	}
	else{

		TFile f1("../dataPoints/V0s_PbPb_Cent.root","RECREATE");
	}
