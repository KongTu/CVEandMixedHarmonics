...
private:
  ...
  // ----------member data ---------------------------
    ...
    TH3D* hitMap_K0s_matched_dau1;
    TH3D* hitMap_K0s_matched_dau2;

    TH3D* hitMap_Lam_matched_dau1;
    TH3D* hitMap_Lam_matched_dau2;
};
        ...
        if( matchedK0s ){
            ...
            for (int i=0; i<p1.numberOfHits(hitCat); i++){
                ...
                if (p1.validHitFilter(hit) && p1.pixelBarrelHitFilter(hit)){
                    ...
                    hitMap_K0s_matched_dau1->Fill(dau1_algo, pixelLayer, dau1->pt());
                }

                if (p1.validHitFilter(hit) && p1.stripHitFilter(hit)){
     
                    int stripLayer = p1.getLayer(hit)+4;
                    hitMap_K0s_matched_dau1->Fill(dau1_algo, stripLayer, dau1->pt());
                }
            ...
            for (int i=0; i<p2.numberOfHits(hitCat); i++){
                ...
                if (p2.validHitFilter(hit) && p2.pixelBarrelHitFilter(hit)){
                    ...
                    hitMap_K0s_matched_dau2->Fill(dau2_algo, pixelLayer, dau2->pt());
                }

                if (p2.validHitFilter(hit) && p2.stripHitFilter(hit)){
     
                    int stripLayer = p2.getLayer(hit)+4;
                    hitMap_K0s_matched_dau2->Fill(dau2_algo, stripLayer, dau2->pt());
                }
        ...
        if( matchedLam ){
            ...
            for (int i=0; i<p1.numberOfHits(hitCat); i++){
                ...
                if (p1.validHitFilter(hit) && p1.pixelBarrelHitFilter(hit)){
                    ...
                    int pixelLayer = p1.getLayer(hit);
                    hitMap_Lam_matched_dau1->Fill(dau1_algo, pixelLayer, dau1->pt());
                }

                if (p1.validHitFilter(hit) && p1.stripHitFilter(hit)){
     
                    int stripLayer = p1.getLayer(hit)+4;
                    hitMap_Lam_matched_dau1->Fill(dau1_algo, stripLayer, dau1->pt());
                }
            ...
            for (int i=0; i<p2.numberOfHits(hitCat); i++){
                ...
                if (p2.validHitFilter(hit) && p2.pixelBarrelHitFilter(hit)){
                    ...
                    int pixelLayer = p2.getLayer(hit);
                    hitMap_Lam_matched_dau2->Fill(dau2_algo, pixelLayer, dau2->pt());
                }

                if (p2.validHitFilter(hit) && p2.stripHitFilter(hit)){
     
                    int stripLayer = p2.getLayer(hit)+4;
                    hitMap_Lam_matched_dau2->Fill(dau2_algo, stripLayer, dau2->pt());
                }
...
V0AnalyzerSimpleNtuple::beginJob()
{
    ...
    hitMap_K0s_matched_dau1 = fs->make<TH2D>("hitMap_K0s_matched_dau1",";algo;hit_layer",30,0,30,30,0,30);
    hitMap_K0s_matched_dau2 = fs->make<TH2D>("hitMap_K0s_matched_dau2",";algo;hit_layer",30,0,30,30,0,30);
    hitMap_Lam_matched_dau1 = fs->make<TH2D>("hitMap_Lam_matched_dau1",";algo;hit_layer",30,0,30,30,0,30);
    hitMap_Lam_matched_dau2 = fs->make<TH2D>("hitMap_Lam_matched_dau2",";algo;hit_layer",30,0,30,30,0,30);
